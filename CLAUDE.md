...
| Typed errors (10 `HookKitError` subclasses, `error` annotation kind) | None (engine catches and annotates) |
...
- `tests/` — the regular unit + integration suite (the bulk of the 821 tests across all three suites). The coverage-floor enforcer parses its `--coverage` output.
