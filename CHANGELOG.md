...
- **`examples/escalation-listener-stdout/`.** New worked-example directory: ~40-line stdout-prompt listener (`src/listener.ts`) that polls a session spool, prints the pending request to stdout, and reads an `allow|deny` decision from stdin. README + `package.json` + symlinked `node_modules/@questi0nm4rk/hook-kit` (per L-M1.3-2). Downstream consumers fork this for Slack / IDE / webhook integrations. Referenced from `docs/ESCALATION.md` § Listener authoring. [TASK-040]
