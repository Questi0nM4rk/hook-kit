...
### M1.5 — StateStore concurrency contract (in flight)

- **`StateStore` contract JSDoc.** New comprehensive JSDoc block above the `StateStore` interface declaration in `src/core/types.ts` enumerating the four guarantees every conforming implementation must honour: atomicity, flush durability, concurrent stores (last-write-wins per key), and no multi-key transactions. The block is the inline contract summary; the full discussion with worked examples lives in the new `docs/STATE.md`. `MemoryStore` class JSDoc adds the explicit "single-process only; process-wide locks satisfy the contract trivially" wording. [TASK-042 / TASK-046]
- **`docs/STATE.md`.** New top-level contract doc covering StateStore end-to-end so authors of custom stores (SQLite / Redis / in-house) and `stateful()` rule consumers don't need to read source for the semantics. Structure mirrors `docs/ADAPTERS.md` / `docs/ESCALATION.md`: § Contract reproduces the 4 guarantees with worked TypeScript examples per guarantee; § Read-modify-write pattern shows the safe retry-on-conflict shape and the unsafe bare-get-then-set anti-pattern with its silent lost-update; § Per-store guarantees has the comparison table for `MemoryStore` / `TmpdirStore` / `SqliteStateStore` (M2.1) across atomicity / flush durability / concurrent stores / multi-process / Windows. Cross-references SPEC.md § State Management and § Operational Readiness for the engine's fail-open boundary on flush failures. [TASK-043 / TASK-044 / TASK-045]
- **TmpdirStore concurrency decision recorded.** New `docs/specs/tmpdir-store-decision.md` documents the M1.5 design call: `TmpdirStore` stays single-process by design (Option B, demote) rather than gaining `flock`-based file locking (Option A, upgrade). Rationale: multi-process file locking is non-trivial on Windows (no flock(2); needs LockFileEx divergence), `SqliteStateStore` (M2.1) is the principled multi-process answer with WAL mode + busy-timeout giving correct ACID, current TmpdirStore usage is single-process by construction (file path encodes namespace+sessionId), and a visible runtime warning beats silent corruption. [TASK-047]
- **TmpdirStore same-path runtime warning.** Per the Option B decision: same-process detection via a module-level `Map<string, Set<TmpdirStore>>` of open paths. The constructor emits a one-time `console.warn` when a second instance opens a path another `TmpdirStore` has already opened in the same process. Once per (path, process); third + subsequent opens stay silent. Warning text points consumers at `docs/STATE.md` § Per-store guarantees and `SqliteStateStore` (M2.1) as the recommended migration. Cross-process violations are NOT detected (no detection mechanism without locking, which Option B declined) — documented in class JSDoc as the explicit scope. New test-only `__resetOpenPathsForTests` (tagged `@internal`, not exported from the public barrel). Class JSDoc on `TmpdirStore` expanded to enumerate contract scope. [TASK-049 / TASK-048 skipped per decision]
- **`tests/integration/state-e2e.test.ts`.** Triple-window e2e suite (per L-M1.3-5) validating M1.5 StateStore contract + M1.1 DecisionObserver + M1.2 ProtocolAdapter contract in one place. Four cases: (1) happy-path counter across 3 invocations against `mockState`, observer captures only the threshold-cross warning; (2) flush failure — `mockState` flushFn throws → engine's `notifyEngineError("state-flush", err)` path → `StateStoreError` annotation + observer record with `decision: "error"` + ruleId `<engine>:state-flush`; (3) last-write-wins on shared in-process store across two stateful rules; (4) restart recovery — `TmpdirStore` flush + new instance round-trips state across simulated process restart. Library-mode; runs in 90ms wall clock. Test count: tests/ 605 → 609 (+4 e2e). Coverage rose 86.14/89.97 → 86.80/90.07.

### M1.4 — Escalation contract spec (in flight)

- **`ProtocolVersionError` (10th `HookKitError` subclass).** New STABLE-tier typed-error class in `src/core/errors.ts` covering envelope-protocol-version mismatch. `parseAskRequest` pre-validates the `version` field against `PROTOCOL_VERSION` and throws `ProtocolVersionError` directly — version skew (e.g. a v1 client hitting a v2 broker, or a v3 client hitting a v2 broker) surfaces as this specific class instead of the generic `EnvelopeValidationError`. The broker's `parseAskRequest` catch block preserves the `ProtocolVersionError` class (does NOT re-wrap as `EnvelopeValidationError`) and synthesizes a deny reason of the form `[hook-kit broker] protocol version mismatch: envelope protocol version mismatch at parseAskRequest: expected 2, got <actual>` so observability layers can route version skew separately. Re-exported from `src/index.ts` as `@stable @since 1.0.0`. New `HookKitErrorCode` enum entry `"ProtocolVersionError"`. New `tests/escalation/broker-version.test.ts` exercises the three cases (v1 legacy rejected, v99 future rejected, v2 current accepted) at both the `parseAskRequest` boundary and through `brokerAskpass` end-to-end. CLAUDE.md and `docs/ADAPTERS.md` updated to enumerate 10 subclasses. [TASK-037]
- **`docs/ESCALATION.md`.** New contract spec covering the askpass envelope schema (every `AskRequest` + `AskResponse` field quoted verbatim from `src/escalation/envelope.ts`, with type / optionality / semantics / autofill behavior), the broker's filesystem-spool protocol (per-session `~/.cache/hook-kit/sessions/$SESSION_ID/{pending,decided,listeners,meta.json,audit.jsonl}` layout, atomic `O_EXCL` write semantics, NOT line-delimited JSON or Unix sockets), the 5-phase lifecycle (ensureSession → register listener → atomic pending write → poll decided/ → cleanup), the listener-authoring guide (the 3 things every listener does: detect pending, decide-or-forward, submit), tree semantics (`parentSessionId` chain walk + NO PARENT ATTACHED validator + `forwardUp` semantics + harness-ask fall-through at root), and the `HOOK_KIT_ASKPASS` env-var contract. Marked STABLE. Cross-referenced from `docs/SPEC.md` § Escalation. [TASK-034 / 036 / 038 / 039 / 041]
- **`tests/escalation/envelope-schema.test.ts`.** Snapshot test using `z.toJSONSchema(EnvelopeSchema)` (Zod 4 API) to capture the JSON-schema form of `AskRequest` / `AskResponse`. CI fails on accidental shape changes; intentional changes require regenerating the snapshot file in the same commit as the schema change. [TASK-035]
- **`examples/escalation-listener-stdout/`.** New worked-example directory: ~60-line stdout-prompt listener (`src/listener.ts`) that polls a session spool, prints the pending request to stdout, and reads an `allow|deny` decision from stdin. README + `package.json` + symlinked `node_modules/@questi0nm4rk/hook-kit` (per L-M1.3-2). Downstream consumers fork this for Slack / IDE / webhook integrations. Referenced from `docs/ESCALATION.md` § Listener authoring. [TASK-040]
- **`tests/integration/escalation-e2e.test.ts`.** Triple-window e2e suite (per L-M1.3-5) validating M1.4 escalation protocol + M1.1 DecisionObserver + M1.2 ProtocolAdapter contract in one place. Five cases: (1) happy-path ask → mock askpass decides "allow" → outcome resolved + observer captured the ask record; (2) deny-path ask → mock askpass decides "deny" → outcome terminal is deny + observer captured pre-escalation ask; (3) askpass absent → falls through to harness-ask + observer fired; (4) protocol_version mismatch through the broker → `ProtocolVersionError` surfaces in the deny reason; (5) observer record's `decision` is "ask" (pre-escalation) even when the askpass resolves to a different terminal.

### M1.3 — Adapter template (in flight)

- **`examples/adapter-template/`.** New top-level example directory: a fork-and-modify scaffold for authoring a custom `ProtocolAdapter`. Contents: `README.md` (Fork this / What to change / Testing / Wiring into harness X sections, cross-referencing `docs/ADAPTERS.md` for the contract), `package.json` (depends on `@questi0nm4rk/hook-kit`), `build.ts` (calls `bun build --compile --bytecode` directly on `src/main.ts` — not `hook-kit build`, since the canonical CLI only knows `shell` and `cc-tools` modes), `src/my-adapter.ts` (99-line `createMyAdapter` factory with injected `stdin`/`stdout`/`stderr`/`exit` streams so it tests without spawning a process; emits the shell-wrapper output convention from `docs/ADAPTERS.md` § Output convention via shared `formatNonErrorAnnotation` / `formatErrorAnnotation`), `src/parse-input.ts` (wire-format-specific parser separated out per /simplify so README step 1 points at a narrow file), `src/main.ts` (binary entry wiring real `process.*` + an optional `DecisionObserver` from `TEMPLATE_OBSERVER_LOG` env var that appends one JSONL record per decision), `src/hooks.ts` (three STUB demo modules: destructive-rm deny / git force-push ask / no-rf warning — replaced by downstream consumers), `tests/my-adapter.test.ts` (15 in-process unit tests over readInput / writeOutput per-outcome / handleError totality, 100% function coverage; uses `bashEvent` from `@questi0nm4rk/hook-kit/testing` rather than a hand-rolled fixture). Demo rules clearly labeled as stubs. Root `tsconfig.json` extended (`examples/adapter-template` added to `include`); the example resolves `@questi0nm4rk/hook-kit` via a real `node_modules` symlink (gitignored) so devtime tsc + bun + downstream consumers all walk the same resolution path. [TASK-028..033]
- **Annotation formatters re-exported from `@questi0nm4rk/hook-kit`.** `formatNonErrorAnnotation`, `formatErrorAnnotation`, `partitionAnnotations`, `ErrorAnnotation`, and `NonErrorAnnotation` are now part of the public `STABLE @since 1.0.0` API surface (previously internal-only at `src/core/annotations.ts`). Both formatters gain an optional `defaultLabel = "[hook-kit]"` parameter so custom adapters (e.g. `examples/adapter-template`) can override the fallback prefix without re-implementing the line shape. `src/adapters/claude-code.ts` switched its `annotations.map(formatNonErrorAnnotation)` call to an explicit arrow so `Array.map`'s `(element, index)` doesn't accidentally pass the index as the new optional `defaultLabel`. The `[hook-kit]` default keeps the change backward-compatible — all 586 existing tests pass unchanged. [/simplify D]
- **`tests/build/adapter-template-e2e.test.ts`.** Compiled-binary e2e suite validating the wire-format contract from `docs/ADAPTERS.md` § Output convention end-to-end. Stages the template into a tmpdir with symlinks to hook-kit + shell-ast + zod (mirrors `tests/build/example-ai-guardrails.test.ts`), runs the template's own `build.ts`, then spawns the produced binary and pipes synthetic events. Six cases: DENY (rm -rf → exit 2, stderr `[template-demo] denied:`), ASK (git push --force → exit 1, stdout `[template-demo] needs review:`), CLEAN (ls /tmp → exit 0, no output), ANNOTATION-ONLY (rm /tmp/x → exit 0, stdout `[template-demo] warning:`), BAD INPUT (malformed JSON → exit 2, stderr `[template-demo] fatal:`), and OBSERVER (deny event with `TEMPLATE_OBSERVER_LOG` set → one JSONL `DecisionEventRecord` written with `decision: "deny"` and the expected `ruleId`/`ruleKind`/`reason`/`label`/`event.toolInputHash` shape). Validates M1.1 DecisionObserver wiring + M1.2 contract + M1.3 template in one shot. Tests count: tests/ 565 → 586 (+6 e2e + +15 template unit in a separate invocation = +21 total). Root `test` script extended to a third invocation: `bun scripts/check-coverage.ts && bun test tests-isolated/ && bun test examples/adapter-template/tests/`. Coverage floor unchanged (84.95% funcs / 89.54% lines). [TASK-031..033 + e2e add-on]

### M1.2 — Adapter contract documentation (in flight)

- **`docs/ADAPTERS.md`.** New top-level doc spec'ing the `ProtocolAdapter` contract end-to-end so third-party adapters (Cursor / Cline / MCP / custom) can be authored without reading source. Four sections: § Contract (per-method signature + lifecycle + input/output contract + worked example for `readInput` / `writeOutput` / `handleError`), § Output convention (the 4-row exit/stream/content table reproduced verbatim from `CLAUDE.md` with the `<prefix>` + merge-policy summary), § Error handling (the 9 `HookKitError` subclasses enumerated with per-class purpose, plus the fail-open engine boundary vs fail-closed security boundary distinction), § Anti-patterns (7 DON'Ts: mutate outcome, catch+swallow, parse stdout for decisions, retain state across cycles, assume `toolName === "Bash"`, mix rule logic into the adapter, `process.exit` from `readInput`). Both reference implementations (`src/adapters/claude-code.ts`, `src/adapters/raw.ts`) gain a two-line top-of-file cross-reference to the spec. [TASK-023..027]
