...
- **`examples/adapter-template/`.** New top-level example directory: a fork-and-modify scaffold for authoring a custom `ProtocolAdapter`. Contents: `README.md` (Fork this / What to change / Testing / Wiring into harness X sections, cross-referencing `docs/ADAPTERS.md` for the contract), `package.json` (depends on `@questi0nm4rk/hook-kit`), `build.ts` (calls `bun build --compile --bytecode` directly on `src/main.ts` — not `hook-kit build`, since the canonical CLI only knows `shell` and `cc-tools` modes), `src/my-adapter.ts` (~150-line `createMyAdapter` factory with injected `stdin`/`stdout`/`stderr`/`exit` streams so it tests without spawning a process; emits the shell-wrapper output convention from `docs/ADAPTERS.md` § Output convention), `src/main.ts` (binary entry wiring real `process.*` + an optional `DecisionObserver` from `TEMPLATE_OBSERVER_LOG` env var that appends one JSONL record per decision), `src/hooks.ts` (three STUB demo modules: destructive-rm deny / git force-push ask / no-rf warning — replaced by downstream consumers), `tests/my-adapter.test.ts` (15 in-process unit tests over readInput / writeOutput per-outcome / handleError totality, 100% function coverage). Demo rules clearly labeled as stubs. Root `tsconfig.json` extended (`examples/adapter-template` added to `include`, `@questi0nm4rk/hook-kit` self-import path mapping added) so tsc sees the new code per L1. [TASK-028..033]
